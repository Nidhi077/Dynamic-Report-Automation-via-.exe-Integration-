...
#%%
# def avg_min_max_stack_v_old(Vcell_data, image_folder_name):
...
#                            x = avg_per_hour['Date'],
#                            y = avg_per_hour['Avg_V_C1_1-C8_16'],
#                            mode = 'markers',
#                            #mode = 'lines',
#                            legendgroup=3,
#                            name = 'L1 (V)',
...
#                            x = avg_per_hour['Date'],
#                            y = avg_per_hour['Min_V_C1_1-C8_16'],
#                            mode = 'markers',
#                            #mode = 'lines',
#                            name = 'L1 (V)',
#                            legendgroup=3,
...
def ASR_chart(Vcell_data, image_folder_name):
    ...
    #avg_per_hour = Vcell_data[['Date','CURRENT DENSITY','ASR_Stack1','ASR(max)_Stack1']]
...
def LWC_Pressure_and_PMP_State(sys_data, image_folder_name):
    avg_per_hour = sys_data.loc[:,['Date','PRT 704', 'PRT 705', 'PMP_ON_OFF']].resample('H', on='Date').mean().reset_index()
    # define a new figure
...
def stack_specific_energy(Vcell_data, image_folder_name):
    ...
    #                                     title = 'Date',
                ...
                yaxis2= dict(title='Current Density'+'<br>' +'(A/c'+'m\u00b2)',
                                     ...
                                     rangemode='tozero'),
    #                                  range=[0,600]),
                legend = dict(xanchor="right",x=1.35,yanchor="top", y =1.3, bgcolor = 'rgba(0,0,0,0)'),
                template = 'simple_white',
                font = dict(size = 18)
                )
...
def avg_min_max_stack_v_old_singleaxis(sys_data, image_folder_name):
    ...
    avg_per_hour = sys_data.loc[:,['Date','CURRENT DENSITY','Avg_V_C1_1-C8_16', 'Max_V_C1_1-C8_16', 'Min_V_C1_1-C8_16','std_dev']].resample('H', on='Date').mean().reset_index()
...
#%% sandbox
# csv_file_list = ['C:/Users/nidhi.dubey/OneDrive - Ohmium International, Inc/Documents/Lotus_system_data/extracted_files_ntpc/lhc1_2023_12_02.csv',
...
# ##avg_per_hour = Vcell_data.groupby(['date_&_hrs'])[['CURRENT DENSITY','STACK SPECIFIC ENERGY','order']].mean().reset_index()    
# ##avg_per_hour = Vcell_data.groupby(Vcell_data['Date'].dt.strftime('%d-%m-%Y %H'))[['CURRENT DENSITY','STACK SPECIFIC ENERGY','order']].mean().reset_index()

# #%%
# # test = Vcell_data.groupby(pd.Grouper(key='Date', freq='M'))
