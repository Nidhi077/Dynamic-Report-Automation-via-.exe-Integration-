...
def create_Vcell_data(sys_data):
    ...
    cell_range = {'L1':List_active_cells}
#    cell_range = {'L1':List_active_cells}    
    ...
    # Vcell_data['Avg_V'] = (Vcell_data[List_active_cells].sum(axis=1))/len(List_active_cells)
    # Vcell_data['Max_V'] = Vcell_data[List_active_cells].max(axis=1) 
    # Vcell_data['Min_V] = Vcell_data[List_active_cells].min(axis=1)
    ...
    #ASR in mohm-cm2
    # Vcell_data['ASR_Stack1'] = (Vcell_data['Avg_V_C1_1-C8_16']/Vcell_data['CURRENT DENSITY'])*1000
    Vcell_data['ASR_Stack1'] = (Vcell_data['Avg_V']/Vcell_data['CURRENT DENSITY'])*1000


    # Vcell_data['ASR(max)_Stack1'] = (Vcell_data['Max_V_C1_1-C8_16']/Vcell_data['CURRENT DENSITY'])*1000
    Vcell_data['ASR(max)_Stack1'] = (Vcell_data['Max_V']/Vcell_data['CURRENT DENSITY'])*1000
...
def ASR_chart(Vcell_data, image_folder_name):
...
def limit_setting(sys_data):
    ...
    Col_List_0to100 = ['PRT 702','PRT 703','PRT 704','PRT 705','PRT 706','KTC 901','HYS 101','HYS 401' ,'HYS 501' ,'HYS 102' ,'OXS 101' ,'LVL 101', 'TTC 102' ,'PRT 401' ,'PRT 402' ,'PRT 403','KTC 401' ,'TTC 101' ,'TTC 301' ,'PRT 101']
...
#%%
def back_pressure_and_current(sys_data, image_folder_name):
    #avg_per_hour = sys_data.loc[:,['STACK CURRENT', 'PRT 401', 'PRT 402', 'PRT 403']].resample('H', on='Date').mean().reset_index()
    sys_data = sys_data[sys_data['STACK CURRENT'] > 0]
    
    fig = sp.make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03)
    fig.update_layout(
                #title = 'LHC Pressure vs Current chart',
                ...
                xaxis3 = dict(#title = 'Date',
                              ...
                              hoverformat = "%d-%b %H:00",
                              showgrid = False),                                                                                
                yaxis = dict(title='PRT 401'+'<br>'+'(bar)',
    #                                font = 15,
                                    #range = [1.4, 2],
                                    showgrid = False,                     
                                    #rangemode='tozero'
                                    ),
                yaxis2 = dict(title='PRT 403'+'<br>'+'(bar)',
    #                                 font = 15,
                                    #range = [0, None],
                                    showgrid = False,
                                    rangemode='tozero'
                                    ),
                yaxis3 = dict(title='PRT 404'+'<br>'+'(bar)',
    #                                 font = 15,
                                    #range = [1.3, 2.3],
                                    showgrid = False,
                                    #rangemode='tozero'
                ...
                #legend= dict(xanchor="right",x=1.5,yanchor="top", y =2, bgcolor = 'rgba(0,0,0,0)'),               
                #legend= dict(xanchor="right",yanchor="top", y=1.38, bgcolor = 'rgba(0,0,0,0)'),
                legend= dict(xanchor="left",x=0, yanchor="top", y =1.3, bgcolor = 'rgba(0,0,0,0)', orientation='h'),
                template = 'simple_white',
                font = dict(size = 18),
                width=850
                )

    fig.update_layout(xaxis3 = dict(titlefont = dict(size = 18), tickfont = dict(size = 18)),
           yaxis = dict(titlefont = dict(size = 18), tickfont = dict(size = 18)),
           yaxis2 = dict(titlefont = dict(size = 18), tickfont = dict(size = 18)),
           yaxis3 = dict(titlefont = dict(size = 18), tickfont = dict(size = 18))
           )
    Pressure_401 = go.Scattergl(
                           x = sys_data['Date'],
                           y = sys_data['PRT 401'],
    ...
    fig.add_trace(Pressure_401, row=1, col=1)
    
    Pressure_403 = go.Scattergl(
                           x = sys_data['Date'],
                           y = sys_data['PRT 403'],
                           #mode = 'markers',
                           mode = 'lines',
    ...
    fig.add_trace(Pressure_403, row=2, col=1)
    
    Pressure_404 = go.Scattergl(
                           x = sys_data['Date'],
                           y = sys_data['PRT 404'],
                           #mode = 'markers',
                           mode = 'lines',
    ...
    fig.add_trace(Pressure_404, row=3, col=1)
     
    fig.write_image('{}/LHC_Pressure_&_Stack_Current_chart.png'.format(image_folder_name), engine="orca")
    fig.write_html('{}/LHC_Pressure_&_Stack_Current_chart.html'.format(image_folder_name), include_plotlyjs=('cdn'))
    # fig1.show()
    print('exported LHC_Pressure_&_Stack_Current Chart')
# In[16]:
    
def H2purity_massflow_o2ppm_vs_pressure(sys_data, image_folder_name):
    #avg_per_hour = sys_data.loc[:,['STACK CURRENT', 'PRT 401', 'PRT 402', 'PRT 403']].resample('H', on='Date').mean().reset_index()
    #sys_data = sys_data[sys_data['STACK CURRENT'] > 0]
    
    fig = sp.make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03, specs=[[{"secondary_y":True}], [{"secondary_y":True}], [{"secondary_y":True}]])
    fig.update_layout(
                #title = 'LHC Pressure vs Current chart',
                                
                yaxis = dict(title='H2 purity (%)',
    #                                font = 15,
                                    #range = [1.4, 2],
                                    showgrid = False,                     
                                    #rangemode='tozero'
                                    ),
                yaxis2 = dict(title='Pressure'+'<br>'+'(bar)',                                                
                                    #range = [0, None],
                                    showgrid = False,
                                    #rangemode='tozero'
                                    ),
                yaxis3 = dict(title='Mass flow'+'<br>'+'(kg/hr)',                                                
                                    #range = [0, None],
                                    showgrid = False,
                                    #rangemode='tozero'
                                    ),
                yaxis4 = dict(title='Pressure'+'<br>'+'(bar)',                                                
                                    #range = [0, None],
                                    showgrid = False,
                                    #rangemode='tozero'
                                    ),
                yaxis5 = dict(title='O2'+"<br>" +'(ppm)',                                               
                                    #range = [0, None],
                                    showgrid = False
                                    #rangemode='tozero'
                                    ),
                yaxis6 = dict(title='Pressure'+'<br>'+'(bar)',                                               
                                    #range = [0, None],
                                    showgrid = False,
                                    #rangemode='tozero'
                                    ),
                #legend= dict(xanchor="right",x=1.5,yanchor="top", y =2, bgcolor = 'rgba(0,0,0,0)'),               
                #legend= dict(xanchor="right",yanchor="top", y=1.38, bgcolor = 'rgba(0,0,0,0)'),
                legend= dict(xanchor="left",x=0, yanchor="top", y =1.2, bgcolor = 'rgba(0,0,0,0)', orientation='h'),
                template = 'simple_white',
                font = dict(size = 16),
                width=850
                )

    fig.update_layout(xaxis2 = dict(titlefont = dict(size = 16), tickfont = dict(size = 16)),
            yaxis = dict(titlefont = dict(size = 16), tickfont = dict(size = 16)),
            yaxis2 = dict(titlefont = dict(size = 16), tickfont = dict(size = 16)),
            yaxis3 = dict(titlefont = dict(size = 16), tickfont = dict(size = 16)),
            yaxis4 = dict(titlefont = dict(size = 16), tickfont = dict(size = 16)),
            yaxis5 = dict(titlefont = dict(size = 16), tickfont = dict(size = 16)),
            yaxis6 = dict(titlefont = dict(size = 16), tickfont = dict(size = 16))
)
    SYS_PRT = go.Scattergl(
                           x = sys_data['Date'],
                           y = sys_data['SYS PRT'],
                           mode = 'markers',
                           #mode = 'lines',
    ...
    fig.add_trace(SYS_PRT, row=1, col=1, secondary_y=True)
    
    H2_purity = go.Scattergl(
                           x = sys_data['Date'],
                           y = sys_data['SYS H2 PURITY'],
                           mode = 'markers',
                           #mode = 'lines',
                           name = 'H2 purity',
                           #showlegend=False,
    ...
    fig.add_trace(H2_purity, row=1, col=1)
    ...
    fig.add_trace(SYS_PRT, row=2, col=1, secondary_y=True)
    
    mass_flow = go.Scattergl(
                            x = sys_data['Date'],
                            y = sys_data['SYS FLW'],
                            mode = 'markers',
                            #showlegend=False,
    ...
    fig.add_trace(mass_flow, row=2, col=1)
    ...
    fig.add_trace(SYS_PRT, row=3, col=1, secondary_y=True)
    ...
    fig.add_trace(O2_ppm, row=3, col=1)
    
    
        
    fig.write_image('{}/LHC_H2purity_massflow_O2ppm_.png'.format(image_folder_name), engine="orca")
    fig.write_html('{}/LHC_H2purity_massflow_O2ppm.html'.format(image_folder_name), include_plotlyjs=('cdn'))
    # fig1.show()
    print('exported LHC_H2purity_massflow_O2ppm')   
#%%
def dewpoint_vs_pressure (sys_data, image_folder_name):
    #avg_per_hour = sys_data.loc[:,['STACK CURRENT', 'PRT 401', 'PRT 402', 'PRT 403']].resample('H', on='Date').mean().reset_index()
    sys_data = sys_data[sys_data['TTC 102'] > 60]
    sys_data = sys_data[sys_data['PRT 401'] > 0]
    
    #fig = sp.make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.03)
    SYS_DPT = go.Scattergl(
                           x = sys_data['Date'],
                           y = sys_data['SYS DPT'],
                           #mode = 'markers',
                           #mode = 'lines',
    ...
    #                                     title = 'Date',
                ...
                yaxis = dict(title='Dewpoint (deg C)',
    #                                 font = 15,
                                    showgrid = False,
                                    #rangemode='tozero'
                                    ),
    ...
    fig1.write_image('{}/LHC_Dewpoint_pressure_Chart.png'.format(image_folder_name), engine="orca")
    fig1.write_html('{}/LHC_Dewpoint_pressure_Chart.html'.format(image_folder_name), include_plotlyjs=('cdn'))
    # fig1.show()
    print('exported LHC_Dewpoint_pressure Chart')   
...
def stack_specific_energy_new(sys_data, image_folder_name):
    # No_of_Stacks = len(Stack_voltage_list)    
    sys_data['STACK VOLTAGE'] = sys_data[Stack_voltage_list].sum(axis=1)
    # print(sys_data[sys_data['Date'] == '05-11-2023 12:26:40'][['Date','STACK VOLTAGE']])

    sys_data['no_of_stacks'] = sys_data[Stack_voltage_list].count(axis=1)

    ## if no of stacks is not a multiple of 56 , then drop that row (based on the fact that there will be always 56 stacks present in the system)
    sys_data  = sys_data[sys_data['no_of_stacks']%56 == 0]

    # sys_data['STACK SPECIFIC ENERGY'] = sys_data['STACK VOLTAGE']/(0.04*No_of_Stacks) [this was old calculation which was corrected later]
    sys_data['STACK SPECIFIC ENERGY'] = sys_data['STACK VOLTAGE']/(0.0376*sys_data['no_of_stacks'])
    # print(sys_data[sys_data['Date'] == '05-11-2023 12:26:40'][['Date','STACK SPECIFIC ENERGY']])
...
def avg_min_max_stack_v_old_singleaxis(sys_data, image_folder_name):
    ...
    avg_per_hour = sys_data.loc[:,['Date','CURRENT DENSITY','Avg_V', 'Max_V', 'Min_V','std_dev']].resample('H', on='Date').mean().reset_index()
    ...
    print('exported LHC_Stack_Voltage chart')
#%%
def avg_min_max_stack_v_Ben(sys_data, image_folder_name):

    #avg_per_hour = Vcell_data.loc[:,['Date','CURRENT DENSITY','Avg_V', 'Max_V', 'Min_V']].resample('H', on='Date').mean().reset_index()
    ...
    #                        mode = 'lines',
                           name = 'Cd',
                           marker=dict(color = 'rgb(214, 39, 40)',size=3),
                           yaxis='y2'
                           #text = df.column (this is the info displayed on hover)
                           )
    ...
    std_dev_trace = go.Scattergl(
                            ...
                            name = 'Std Dev',
                            #line = dict(color = 'rgb(255, 127, 14)', shape='hv'),
                            marker=dict(size=3, color='rgb(68, 170, 153)'),
                            yaxis='y2'
                            )
    #adding cd trace to the figure
...
#     print(csv_file_list)
    ...
    sys_data['Avg_V'] = (sys_data[List_active_cells].sum(axis=1))/len(List_active_cells)
    sys_data['Max_V'] = sys_data[List_active_cells].max(axis=1) 
    sys_data['Min_V'] = sys_data[List_active_cells].min(axis=1)
...
# running the functions for creating dfs & charts
    ...
    # plot_cd(Vcell_data, image_folder_name)
    #avg_min_max_stack_v(sys_data, image_folder_name)
    #avg_min_max_stack_v_old_singleaxis(sys_data, image_folder_name)
    avg_min_max_stack_v_Ben(sys_data, image_folder_name)
    #avg_min_max_stack_v_old(sys_data, image_folder_name)
    # ASR_chart(Vcell_data, image_folder_name)
    sys_data = limit_setting(sys_data)
    pressure_and_conductivity(sys_data, image_folder_name)
    back_pressure_and_current(sys_data, image_folder_name)
    H2purity_massflow_o2ppm_vs_pressure(sys_data, image_folder_name)
    dewpoint_vs_pressure (sys_data, image_folder_name)
